...
wtd.table2 <- function (x, weights = NULL, type = c("list", "table"), normwt = FALSE, 
    na.rm = TRUE) {
    type <- match.arg(type)
    if (!length(weights)) 
...
wtd.quantile2 <- function(x, weights = NULL, probs = c(0, 0.25, 0.5, 0.75, 1), 
    ...
    if (type == "quantile") {
        w <- wtd.table2(x, weights, na.rm = na.rm, normwt = normwt, 
            type = "list")
        x <- w$x
...
matchWeightedUpperQuantileForVariance <- function(x, weights, upperQuantile=.05) {
  sdEst <- wtd.quantile2(abs(x), weights=weights, 1 - upperQuantile, normwt=TRUE) / qnorm(1 - upperQuantile/2)
  unname(sdEst)^2
}
